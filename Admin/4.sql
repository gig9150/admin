...
DROP TABLE IF EXISTS member_penalty_update;
...
DROP TABLE IF EXISTS shop_review_like;
...
DROP TABLE IF EXISTS franchise_and_question;
...
DROP TABLE IF EXISTS notice_franchise;
...
DROP TABLE IF EXISTS prohi_word;
...
DROP TABLE IF EXISTS shop_news_category;
...
CREATE TABLE franchise_and_question
(
	franchise_and_question_id int(11) NOT NULL AUTO_INCREMENT,
	-- 프렌차이즈 번호 
	-- 
	franchise_id int(11) NOT NULL COMMENT '프렌차이즈 번호 
',
	-- 자주묻는 질문 번호
	frequent_question_id int(11) NOT NULL COMMENT '자주묻는 질문 번호',
	PRIMARY KEY (franchise_and_question_id)
);
...
(
	...
	-- 노출 상태
	-- 1 : 노출 / 비 노출 
	view_status int(11) NOT NULL COMMENT '노출 상태
1 : 노출 / 비 노출 ',
	-- 상단 고정 여부 1 : 고정 / 2 : 비고정
	fixed_status int(11) COMMENT '상단 고정 여부 1 : 고정 / 2 : 비고정',
	-- 등록일
...
(
	...
	-- 정가
	price varchar(300) COMMENT '정가',
	-- 최소금액
	min_price varchar(300) COMMENT '최소금액',
	-- 추가 설명(선택)
	additional_info  text COMMENT '추가 설명(선택)',
	-- 1 : 베스트 2 : 아님
	best int(5) COMMENT '1 : 베스트 2 : 아님',
...
(
	...
	-- 널인지 체크
	-- 
	check_null varchar(300) COMMENT '널인지 체크
',
...
(
	...
	-- 2:정지
	-- 3:차단
	penalty_name int(5) COMMENT '패널티 종류
1:경고
2:정지
3:차단',
	-- 패널티 구분
...
1:  계정 / 2: 상품 / 3: 채팅 / 4: 게시글',
	...
	-- 1 : 삭제 / 2 : 미삭제 / 3 : 존재하지않음
	-- 
	delete_state int(5) COMMENT '게시글 삭제 여부
1 : 삭제 / 2 : 미삭제 / 3 : 존재하지않음
',
	-- 발견 경로
	--  1:사전 모니터링 / 2: 신고
 ...
 1:사전 모니터링 / 2: 신고',
	...
	-- 임시 패널티 종료 날짜
	-- (패널티가 해제되면 원래 종료날짜 저장)
	tmp_endDate timestamp COMMENT '임시 패널티 종료 날짜
(패널티가 해제되면 원래 종료날짜 저장)',
	-- 등록일
	-- 
	regdate timestamp COMMENT '등록일
...
CREATE TABLE member_penalty_update
(
	member_penalty_update_id int(11) NOT NULL AUTO_INCREMENT,
	-- 회원 패널티 번호
	member_penalty_id int(11) NOT NULL COMMENT '회원 패널티 번호',
	-- 수정일 
	regdate timestamp COMMENT '수정일 ',
	-- 패널티 종류 
	-- 1 : 경고
	-- 2 : 정지
	-- 3 : 차단
...
',
	-- 패널티 적용일 
	startDate timestamp COMMENT '패널티 적용일 ',
	-- 패널티 종료일
	endDate timestamp COMMENT '패널티 종료일',
	-- 사유 
	...
	-- 2 : 해지
	-- 
	state int(5) COMMENT '패널티 상태
1 : 적용
2 : 해지
',
	-- 처리자 
	register varchar(300) COMMENT '처리자 ',
	PRIMARY KEY (member_penalty_update_id)
);
...
CREATE TABLE news_update_list
(
	-- 소식 수정 이력 번호
	news_update_list_id int(11) NOT NULL AUTO_INCREMENT COMMENT '소식 수정 이력 번호',
	-- 가게 소식 번호
	...
	-- 수정 항목 
	-- 
	update_category varchar(300) COMMENT '수정 항목 
',
	-- 수정 전
	update_before varchar(300) COMMENT '수정 전',
	-- 수정 후 
	update_after varchar(300) COMMENT '수정 후 ',
	-- 진열 상태(삭제 상태 / 관리자 삭제와 대표자,매니저 삭제 구분)
	-- 
	-- 1 : 진열
	-- 2 : 미진열
	-- 
	post_status int(11) COMMENT '진열 상태(삭제 상태 / 관리자 삭제와 대표자,매니저 삭제 구분)
...
',
	...
	-- 2 : 정상
	-- 
	manager_del int(11) COMMENT '관리자 삭제 여부
1 : 관리자 삭제
2 : 정상
',
	-- 처리자
	handler varchar(300) COMMENT '처리자',
	PRIMARY KEY (news_update_list_id)
);
...
(
	...
	-- 1 : 노출 / 2: 미 노출
	-- 
	view_status int(11) COMMENT '노출 상태 
1 : 노출 / 2: 미 노출
',
	-- 상단 고정 여부
	-- 1 : 고정 / 2 : 미 고정
	fixed_status int(11) COMMENT '상단 고정 여부
1 : 고정 / 2 : 미 고정',
	-- 공지사항 등록 날짜
...
CREATE TABLE notice_franchise
(
	notice_franchise int(11) NOT NULL AUTO_INCREMENT,
	notice_id int(11) NOT NULL,
	-- 프렌차이즈 번호 
	-- 
	franchise_id int(11) NOT NULL COMMENT '프렌차이즈 번호 
',
	PRIMARY KEY (notice_franchise)
);
...
CREATE TABLE prohi_word
(
	prohi_word_id int(11) NOT NULL AUTO_INCREMENT,
	-- 가맹점매니저번호
	franchise_manager_id int(11) NOT NULL COMMENT '가맹점매니저번호',
	-- 글제목 여부
	-- on : 적용
	-- 
	text_title varchar(50) COMMENT '글제목 여부
on : 적용
',
	-- 글 내용 등록 여부
	-- on : 적용
	-- 
	text_content  varchar(50) COMMENT '글 내용 등록 여부
on : 적용
',
	-- 후기내용 적용여부 
	-- on : 적용
	-- 
	review_content varchar(50) COMMENT '후기내용 적용여부 
on : 적용
',
	-- 금지어
	word varchar(300) COMMENT '금지어',
	-- 등록일
	-- 
	regdate timestamp COMMENT '등록일
',
	PRIMARY KEY (prohi_word_id)
);
...
(
	...
	-- 1 : 소식 글 신고 
	-- 
	category_sub int(11) COMMENT '하위 카테고리
1 : 소식 글 신고 
',
	-- 카테고리 내용
...
',
	...
	-- 가게 탈퇴사유 카테고리 번호
	shop_with_category_id int(11) NOT NULL COMMENT '가게 탈퇴사유 카테고리 번호',
	-- 탈퇴사유
	with_content  varchar(300) COMMENT '탈퇴사유',
	-- 1:활동중
	-- 2:중지
	-- 3:탈퇴 
	-- 
...
CREATE TABLE shop_hold
(
	shop_hold_id int(11) NOT NULL AUTO_INCREMENT,
	shop_idx int(11) NOT NULL,
	-- 사유
	-- 
	reason varchar(300) COMMENT '사유
',
	...
	-- 처리일
	-- 
	regdate timestamp COMMENT '처리일
',
	PRIMARY KEY (shop_hold_id)
);
...
(
	...
	shop_idx int(11) NOT NULL,
	-- 카테고리 번호
	shop_news_category_id int(11) NOT NULL COMMENT '카테고리 번호',
	-- 가게 소식 제목 
	...
	-- 1 : 대기 2: 승인 3: 거절
	-- 
	post_stage int(11) COMMENT '게시 단계(사전 모니터링)
1 : 대기 2: 승인 3: 거절
',
	-- 진열 상태(삭제 상태 / 관리자 삭제와 대표자,매니저 삭제 구분)
	-- 
...
',
	...
	-- 1 : 대표자 or 매니저 삭제
	-- 2 : 정상 
	member_del int(11) COMMENT '대표자 or 매니저 삭제
1 : 대표자 or 매니저 삭제
2 : 정상 ',
	-- 삭제일
	del_regdate timestamp COMMENT '삭제일',
	PRIMARY KEY (shop_news_id)
);


CREATE TABLE shop_news_category
(
	-- 카테고리 번호
	shop_news_category_id int(11) NOT NULL AUTO_INCREMENT COMMENT '카테고리 번호',
	-- 카테고리 내용
	cate_content varchar(300) COMMENT '카테고리 내용',
	PRIMARY KEY (shop_news_category_id)
);
...
(
	...
	-- 2:정지
	-- 3:차단
	panalty_name  int(5) COMMENT '패널티 종류
1:경고
2:정지
3:차단',
	-- 패널티 구분
...
CREATE TABLE shop_penalty_update
(
	shop_penalty_update_id int(11) NOT NULL AUTO_INCREMENT,
	-- 가게 패널티 번호 
	shop_penalty_id int(11) NOT NULL COMMENT '가게 패널티 번호 ',
	-- 수정일
	-- 
	regdate timestamp COMMENT '수정일
',
	-- 패널티 종류 1 : 경고 / 2 : 정지 / 3 : 차단
	penalty_name int(5) COMMENT '패널티 종류 1 : 경고 / 2 : 정지 / 3 : 차단',
	-- 패널티 적용일
	startDate timestamp COMMENT '패널티 적용일',
	-- 패널티 종료일
	endDate timestamp COMMENT '패널티 종료일',
...
',
	-- 처리자
	register varchar(300) COMMENT '처리자',
	PRIMARY KEY (shop_penalty_update_id)
);
...
(
	...
	view_count int(11) COMMENT '조회수 ',
	-- 사장님 댓글 
	reply varchar(300) COMMENT '사장님 댓글 ',
...
CREATE TABLE shop_review_like
(
	-- 가게 후기 좋아요 번호
	shop_review_like_id int(11) NOT NULL AUTO_INCREMENT COMMENT '가게 후기 좋아요 번호',
	-- 회원코드
	member_id int(11) NOT NULL COMMENT '회원코드',
	-- 가게 리뷰 번호 
	shop_review_id int(11) NOT NULL COMMENT '가게 리뷰 번호 ',
	PRIMARY KEY (shop_review_like_id)
);
...
(
	-- 가게 탈퇴사유 카테고리 번호
	shop_with_category_id int(11) NOT NULL AUTO_INCREMENT COMMENT '가게 탈퇴사유 카테고리 번호',
	-- 내용
	-- 
	content varchar(300) COMMENT '내용
',
	PRIMARY KEY (shop_with_category_id)
);
...
ALTER TABLE franchise_and_question
	ADD FOREIGN KEY (franchise_id)
	REFERENCES franchise (franchise_id)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
...
ALTER TABLE franchise_and_question
	ADD FOREIGN KEY (frequent_question_id)
	REFERENCES frequent_question (frequent_question_id)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
...
ALTER TABLE member_penalty_update
	ADD FOREIGN KEY (member_penalty_id)
	REFERENCES member_penalty (member_penalty_id)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
...
ALTER TABLE shop_news
	ADD FOREIGN KEY (shop_news_category_id)
	REFERENCES shop_news_category (shop_news_category_id)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;


ALTER TABLE shop_penalty_update
	ADD FOREIGN KEY (shop_penalty_id)
	REFERENCES shop_penalty (shop_penalty_id)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
...
ALTER TABLE shop
	ADD FOREIGN KEY (shop_with_category_id)
	REFERENCES shop_with_category (shop_with_category_id)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
