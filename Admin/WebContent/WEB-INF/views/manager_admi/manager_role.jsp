            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <form method="get" action="${root}/manager_admi/manager_role">
							...
							<div class="card-body">
								...
								<div class="row">
									...
									<div class="col-sm-6">
										<input type="text" class="form-control" name="registrar" value="${map.registrar}">
									</div>
								</div>
								<div class="row">
									...
									<div class="col-sm-2">
										<select class="form-control" name="select">
											<option value="0" <c:if test="${map.select eq 0}">selected</c:if>>전  체</option>
											<option value="1" <c:if test="${map.select eq 1}">selected</c:if>>그룹 명</option>
											<option value="2" <c:if test="${map.select eq 2}">selected</c:if>>그룹 설명</option>
										</select>
									</div>
									<div class="col-sm-9">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table question-table" id="franchiseListTable" style="margin-top:1rem;">
                                ...
                                <tbody>
                                	<c:forEach items="${list}" var="obj">
	                                	<tr data-role-group-id = '${obj.role_group_id}'>
	                                		<td class="tableTd"><input type="checkbox" class="tableChk"></td>
	                                		<td>${obj.role_group_id}</td>
	                                		<td>${obj.regdate }</td>
	                                		<td colspan="3">${obj.role_group_name}</td>
	                                		<td>-</td>
											<td>-</td>
	                                		<td>${obj.registrar}</td>
	                                		<td class="deleteTd"><button type="button" class="btn btn-secondary delete-btn" data-role-group-id = "${obj.role_group_id}">삭제</button></td>
	                                	</tr>
                                	</c:forEach>
                                </tbody>
                              </table>
                              ...
                              <div class="row justify-content-md-center">
				          		...
				          		<c:choose>
				          			...
				          			<c:otherwise>
				          				<li class="page-item">
				          				<a class="page-link" href="${root}/manager_admi/manager_role?page=${pageBean.prevPage}&selectNum=${selectNum}&startDate=${map.startDate}&endDate=${map.endDate}&registrar=${map.registrar}&select=${map.select}&keyWord=${map.keyWord}" aria-label="Previous">
										        <span aria-hidden="true">&laquo;</span>
										        <span class="sr-only">Previous</span>
									      	</a></li>
				          			</c:otherwise>
				          		...
				          		<c:forEach var="idx" begin="${pageBean.min}" end="${pageBean.max}">
				          			<c:choose>
				          				<c:when test="${idx == pageBean.page}">
				          					<li class="page-item active"><a class="page-link" href="${root}/manager_admi/manager_role?page=${idx}&selectNum=${selectNum}&startDate=${map.startDate}&endDate=${map.endDate}&registrar=${map.registrar}&select=${map.select}&keyWord=${map.keyWord}">${idx}</a></li>
				          				</c:when>
				          				<c:otherwise>
				          					<li class="page-item"><a class="page-link" href="${root}/manager_admi/manager_role?page=${idx}&selectNum=${selectNum}&startDate=${map.startDate}&endDate=${map.endDate}&registrar=${map.registrar}&select=${map.select}&keyWord=${map.keyWord}">${idx}</a></li>
				          				</c:otherwise>
				          			</c:choose>	
				          		</c:forEach>
				          		<c:choose>
				          			...
				          			<c:otherwise>
				          				<li class="page-item">
										      <a class="page-link" href="${root}/manager_admi/manager_role?page=${pageBean.nextPage}&selectNum=${selectNum}&startDate=${map.startDate}&endDate=${map.endDate}&registrar=${map.registrar}&select=${map.select}&keyWord=${map.keyWord}" aria-label="Next">
										        <span aria-hidden="true">&raquo;</span>
										        <span class="sr-only">Next</span>
										      </a>
										    	</li>
						  ...
						  <div style="position: relative;">
						  	<button class="btn btn-info deleteRole" style="position: absolute;left:0;">삭&nbsp;&nbsp;제</button>
						  	<a href="${root}/manager_admi/manager_role_register" class="btn btn-info addRole" style="position:absolute;right:0;">신규등록 </a>
						  </div>
				          <br>
                          <br>
                        </div>
	...
	<script>
		$(function(){
   			...
   			$('#selectNum').on('change',function(){
   				...
   				location.href='${root}/manager_admi/manager_role?startDate=${map.startDate}&endDate=${map.endDate}&registrar=${map.registrar}&select=${map.select}&keyWord=${map.keyWord}&selectNum='+selectNum;
   			});
   			
   			//수정 페이지 이동
   			$('.question-table tbody tr td').not('.tableTd,.deleteTd').on('click',function(){
   				
   				let roleGroupId = $(this).parent().data('roleGroupId');
   				location.href="${root}/manager_admi/manager_role_update?roleGroupId="+roleGroupId;
   			...
   			$('.delete-btn').on('click',function(){
   				let roleGroupId = $(this).data('roleGroupId');
	   			swal({
	   				title : "fail!",
	   				text : "삭제하시겠습니까?",
	   				buttons: {
	   				...
	   				switch(value){
	   					case "defeat" :
	   						swal("삭제되었습니다.").then(function(){
	   							location.href="${root}/manager_admi/manager_role_delete?roleGroupId="+roleGroupId;
	   						});
	   					break;
	   					default :
	   						e.preventDefault();
   			...
   			$('.deleteRole').on('click',function(){
   				...
   				let idList = [];
   				$.each($('tbody .selectedTd'),function(i,v){
   					idList[i] = $(this).data('roleGroupId');
   					parseInt(idList[i]);
   				});
	   			swal({
	   				title : "fail!",
	   				...
	   				switch(value){
	   					case "defeat" :
	   						swal("삭제되었습니다.").then(function(){
	   							location.href="${root}/manager_admi/manager_role_allDelete?idList="+idList.join(',');
	   						});
	   					break;
	   					default :
	   						e.preventDefault();
