        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800"> Home >> 관리자 설정 >> 운영자 계정 관리  </h1>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <form method="get" action="${root}/manager_admi/manager_account">
							...
							<div class="card-body">
								...
								<div class="row">
									...
									<div class="col-sm-4">
										<input type="date" class="form-control" style="width:45%;display:inline-block;" name="startDate" id="startDate" value="${map.startDate}">&nbsp;~&nbsp;
										<input type="date" class="form-control" style="width:45%;display:inline-block;" name="endDate" id="endDate" value="${map.endDate}">
									</div>
									<div class="col-sm-1">가맹사</div>
									<div class="col-sm-6">
										<label><input type="checkbox" class="franAllChk">&nbsp;모두 선택</label>
										<c:forEach items="${franList}" var="obj">
   											<label><input type="checkbox" name="franchiseId" class="franChk" value="${obj.franchise_id}" 
   												<c:forEach items="${map.franchiseId}" var="obj2">
   													<c:if test="${obj2 eq obj.franchise_id}">
   														checked
   													</c:if>
   												</c:forEach>
   											>&nbsp;${obj.franchise_name}</label>
   										</c:forEach>
									</div>
								</div>
								<div class="row">
									...
									<div class="col-sm-2">
										<select class="form-control" name="select">
											<option value="100" <c:if test="${map.select eq 100}">selected</c:if>>전  체</option>
											<option value="1" <c:if test="${map.select eq 1}">selected</c:if>>아이디</option>
											<option value="2" <c:if test="${map.select eq 2}">selected</c:if>>이  름</option>
										</select>
									</div>
									<div class="col-sm-9">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table question-table" id="franchiseListTable" style="margin-top:1rem;">
                                ...
                                <tbody>	
                                	<c:forEach items="${list}" var="obj">
	                                	<tr data-franchise-manager-id="${obj.franchise_manager_id}">
	                                		<th>${obj.franchise_manager_id}</th>
	                                		<td>${obj.regdate}</td>
	                                		<td>${obj.id}</td>
	                                		<td>${obj.name}</td>
	                                		<td>
	                                			<c:forEach items="${obj.franList}" var="fran" varStatus="status">
	                                				<c:choose>
                              ...
                              <div class="row justify-content-md-center">
				          		...
				          		<c:choose>
				          			...
				          			<c:otherwise>
				          				<li class="page-item">
				          				<a class="page-link" href="${root}/manager_admi/manager_account?page=${pageBean.prevPage}&selectNum=${selectNum}&startDate=${map.startDate}&endDate=${map.endDate}&select=${map.select}&keyWord=${map.keyWord}<c:forEach items='${map.franchiseId}' var='obj'>&franchiseId=${obj}</c:forEach>" aria-label="Previous">
										        <span aria-hidden="true">&laquo;</span>
										        <span class="sr-only">Previous</span>
									      	</a></li>
				          			</c:otherwise>
				          		...
				          		<c:forEach var="idx" begin="${pageBean.min}" end="${pageBean.max}">
				          			<c:choose>
				          				<c:when test="${idx == pageBean.page}">
				          					<li class="page-item active"><a class="page-link" href="${root}/manager_admi/manager_account?page=${idx}&selectNum=${selectNum}&startDate=${map.startDate}&endDate=${map.endDate}&select=${map.select}&keyWord=${map.keyWord}<c:forEach items='${map.franchiseId}' var='obj'>&franchiseId=${obj}</c:forEach>">${idx}</a></li>
				          				</c:when>
				          				<c:otherwise>
				          					<li class="page-item"><a class="page-link" href="${root}/manager_admi/manager_account?page=${idx}&selectNum=${selectNum}&startDate=${map.startDate}&endDate=${map.endDate}&select=${map.select}&keyWord=${map.keyWord}<c:forEach items='${map.franchiseId}' var='obj'>&franchiseId=${obj}</c:forEach>">${idx}</a></li>
				          				</c:otherwise>
				          			</c:choose>	
				          		</c:forEach>
				          		<c:choose>
				          			...
				          			<c:otherwise>
				          				<li class="page-item">
										      <a class="page-link" href="${root}/manager_admi/manager_account?page=${pageBean.nextPage}&selectNum=${selectNum}&startDate=${map.startDate}&endDate=${map.endDate}&select=${map.select}&keyWord=${map.keyWord}<c:forEach items='${map.franchiseId}' var='obj'>&franchiseId=${obj}</c:forEach>" aria-label="Next">
										        <span aria-hidden="true">&raquo;</span>
										        <span class="sr-only">Next</span>
										      </a>
										    	</li>
						  ...
						  <div style="position: relative;">
						  	<a href="${root}/manager_admi/manager_account_register" class="btn btn-info addRole" style="position:absolute;right:0;">신규등록 </a>
						  </div>
				          <br>
                          <br>
                        </div>
	...
	<script>
		$(function(){
   			...
   			$('#selectNum').on('change',function(){
   				...
   				location.href='${root}/manager_admi/manager_account?startDate=${map.startDate}&endDate=${map.endDate}&select=${map.select}&keyWord=${map.keyWord}<c:forEach items="${map.franchiseId}" var="obj">&franchiseId=${obj}</c:forEach>&selectNum='+selectNum;
   			});
   			
   			//수정 페이지 이동
   			$('.question-table tbody tr td').not('.tableTd,.deleteTd').on('click',function(){
   				
   				let franchiseManagerId = $(this).parent().data('franchiseManagerId');
   				location.href="${root}/manager_admi/manager_account_update?franchiseManagerId="+franchiseManagerId;
