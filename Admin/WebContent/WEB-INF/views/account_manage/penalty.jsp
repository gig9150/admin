			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<ul class="nav nav-tabs" id="myTab" role="tablist">
						<li class="nav-item"><a class="nav-link active" href="${root}/account_manage/penalty">회원 계정</a></li>
						<li class="nav-item"><a class="nav-link" href="${root}/account_manage/shop_penalty_list">가계 계정</a></li>
					</ul>
					...
					<div class="card shadow mb-4">
						...
						<form method="get" action="${root}/account_manage/penalty">
							...
							<div class="card-body">
								...
								<div class="row">
									...
									<div class="col-sm-2">
										<select class="form-control" name="dateSelete">
											<option value="1">적용일</option>
								...
								<div class="row">
									<div class="col-sm-1">패널티 종류</div>
									<div class="col-sm-2">
										<select class="form-control" name="penaltyName">
											<option value="1"
												<c:if test="${bean.penaltyName eq 1}">selected</c:if>>전체</option>
											<option value="2"
												<c:if test="${bean.penaltyName eq 2}">selected</c:if>>경고</option>
											<option value="3"
												<c:if test="${bean.penaltyName eq 3}">selected</c:if>>이용정지</option>
											<option value="4"
												<c:if test="${bean.penaltyName eq 4}">selected</c:if>>차단</option>
										</select>
									</div>
									<div class="col-sm-1">발견경로</div>
									<div class="col-sm-2">
										<select class="form-control" name="path">
											<option value="100"
												<c:if test="${bean.path eq 100}">selected</c:if>>전체</option>
											<option value="1"
												<c:if test="${bean.path eq 1}">selected</c:if>>신고</option>
											<option value="2"
												<c:if test="${bean.path eq 2}">selected</c:if>>모니터링</option>
										</select>
									</div>
									<div class="col-sm-1">적용 상태</div>
									<div class="col-sm-2">
										<select class="form-control" name="state">
											<option value="100"
												<c:if test="${bean.state eq 100}">selected</c:if>>전체</option>
											<option value="1"
												<c:if test="${bean.state eq 1}">selected</c:if>>적용</option>
											<option value="2"
												<c:if test="${bean.state eq 2}">selected</c:if>>해지</option>
										</select>
									</div>
								...
								<div class="row">
									<div class="col-sm-1">회원 정보</div>
									<div class="col-sm-2">
										<select class="form-control" name="select">
											<option value="100"
												<c:if test="${bean.select eq 100}">selected</c:if>>전체</option>
											<option value="1"
												<c:if test="${bean.select eq 1}">selected</c:if>>닉네임</option>
											<option value="2"
												<c:if test="${bean.select eq 2}">selected</c:if>>회원번호</option>
										</select>
									</div>
									<div class="col-sm-2">
							...
							<table class="table question-table" id="franchiseListTable"
								...
								<tbody>
									<c:forEach items="${list}" var="obj" varStatus="status">
										<tr data-member-penalty-id="${obj.member_penalty_id}">
											<td class="tableTd"><input type="checkbox"
												class="tableChk"></td>
											<th>${obj.member_penalty_id}</th>
											<td>${obj.startDate}</td>
											<td>${obj.tmp_endDate}</td>
											<td>${obj.franchise_name}</td>
											<td>${obj.member_id}</td>
											<td>${obj.name}</td>
											<c:choose>
												<c:when test="${obj.discovery_path eq 1}">
													<td>모니터링</td>
												</c:when>
												<c:otherwise>
													<td>신고</td>
											...
											<c:choose>
												<c:when test="${obj.penalty_name eq 1}">
													<td>경고</td>
												</c:when>
												<c:when test="${obj.penalty_name eq 2}">
													<td>이용정지</td>
												</c:when>
												<c:when test="${obj.penalty_name eq 3}">
													<td>차단</td>
												</c:when>
											</c:choose>
											<c:choose>
												<c:when test="${obj.delete_state eq 1}">
													<td>Y</td>
												</c:when>
												<c:when test="${obj.delete_state eq 2}">
													<td>N</td>
												</c:when>
												<c:when test="${obj.delete_state eq 3}">
													<td>-</td>
												</c:when>
											</c:choose>
							...
							<div class="row justify-content-md-center">
								<ul class="pagination">
									<c:choose>
										...
										<c:otherwise>
											<li class="page-item"><a class="page-link"
												href="${root}/account_manage/penalty?page=${pageBean.prevPage}&selectNum=${selectNum}&dateSelete=${bean.dateSelete}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyword=${bean.keyword}&franchiseId=${bean.franchiseId}&id=${bean.id}&penaltyName=${bean.penaltyName}&path=${bean.path}&state=${bean.state}&register=${bean.register}"
												aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
													<span class="sr-only">Previous</span>
											</a></li>
										</c:otherwise>
										...
										<c:choose>
											<c:when test="${idx == pageBean.page}">
												<li class="page-item active"><a class="page-link"
													href="${root}/account_manage/penalty?page=${idx}&selectNum=${selectNum}&dateSelete=${bean.dateSelete}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyword=${bean.keyword}&franchiseId=${bean.franchiseId}&id=${bean.id}&penaltyName=${bean.penaltyName}&path=${bean.path}&state=${bean.state}&register=${bean.register}">${idx}</a></li>
											</c:when>
											<c:otherwise>
												<li class="page-item"><a class="page-link"
													href="${root}/account_manage/penalty?page=${idx}&selectNum=${selectNum}&dateSelete=${bean.dateSelete}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyword=${bean.keyword}&franchiseId=${bean.franchiseId}&id=${bean.id}&penaltyName=${bean.penaltyName}&path=${bean.path}&state=${bean.state}&register=${bean.register}">${idx}</a></li>
									...
									<c:choose>
										...
										<c:otherwise>
											<li class="page-item"><a class="page-link"
												href="${root}/account_manage/penalty?page=${pageBean.nextPage}&selectNum=${selectNum}&dateSelete=${bean.dateSelete}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyword=${bean.keyword}&franchiseId=${bean.franchiseId}&id=${bean.id}&penaltyName=${bean.penaltyName}&path=${bean.path}&state=${bean.state}&register=${bean.register}"
												aria-label="Next"> <span aria-hidden="true">&raquo;</span>
													<span class="sr-only">Next</span>
											</a></li>
										</c:otherwise>
		...
		$(function() {
			...
			$('#selectNum')
					.on(
							...
							function() {
								...
								location.href = '${root}/account_manage/penalty?dateSelete=${bean.dateSelete}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyword=${bean.keyword}&franchiseId=${bean.franchiseId}&id=${bean.id}&penaltyName=${bean.penaltyName}&path=${bean.path}&state=${bean.state}&register=${bean.register}&selectNum='
										+ selectNum;
							});
			...
			$('.question-table tbody tr td').not('.tableTd').on(
					...
					function() {
						...
						let memberPenaltyId = $(this).parent().data('memberPenaltyId');
						window.open(
								"${root}/account_manage/penalty_detail?memberPenaltyId="
										+ memberPenaltyId, "_blank",
