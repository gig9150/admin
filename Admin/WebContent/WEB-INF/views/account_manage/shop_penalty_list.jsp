			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<ul class="nav nav-tabs" id="myTab" role="tablist">
						<li class="nav-item"><a class="nav-link" href="${root}/account_manage/penalty">회원 계정</a></li>
						<li class="nav-item"><a class="nav-link active" href="${root}/account_manage/shop_penalty_list">가계 계정</a></li>
					</ul>
					...
					<div class="card shadow mb-4">
						...
						<form method="get" action="${root}/account_manage/penalty">
							...
							<div class="card-body">
								...
								<div class="row">
									<div class="col-sm-1">가게 정보</div>
									<div class="col-sm-2">
										<select class="form-control" name="select">
											<option value="100"
												<c:if test="${bean.select eq 100}">selected</c:if>>전체</option>
											<option value="1"
												<c:if test="${bean.select eq 1}">selected</c:if>>가게명</option>
											<option value="2"
												<c:if test="${bean.select eq 2}">selected</c:if>>가게 번호</option>
										</select>
									</div>
									<div class="col-sm-2">
							...
							<table class="table question-table" id="franchiseListTable"
								...
								<tbody>
									<c:forEach items="${list}" var="obj" varStatus="status">
										<tr data-shop-penalty-id="${obj.shop_penalty_id}">
							...
							<div class="row justify-content-md-center">
								<ul class="pagination">
										...
										<c:choose>
											<c:when test="${idx == pageBean.page}">
												<li class="page-item active"><a class="page-link"
													href="${root}/account_manage/shop_penalty_list?page=${idx}&selectNum=${selectNum}&dateSelete=${bean.dateSelete}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyword=${bean.keyword}&franchiseId=${bean.franchiseId}&id=${bean.id}&penaltyName=${bean.penaltyName}&path=${bean.path}&state=${bean.state}&register=${bean.register}">${idx}</a></li>
											</c:when>
											<c:otherwise>
												<li class="page-item"><a class="page-link"
													href="${root}/account_manage/shop_penalty_list?page=${idx}&selectNum=${selectNum}&dateSelete=${bean.dateSelete}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyword=${bean.keyword}&franchiseId=${bean.franchiseId}&id=${bean.id}&penaltyName=${bean.penaltyName}&path=${bean.path}&state=${bean.state}&register=${bean.register}">${idx}</a></li>
									...
									<c:choose>
										...
										<c:otherwise>
											<li class="page-item"><a class="page-link"
												href="${root}/account_manage/shop_penalty_list?page=${pageBean.nextPage}&selectNum=${selectNum}&dateSelete=${bean.dateSelete}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyword=${bean.keyword}&franchiseId=${bean.franchiseId}&id=${bean.id}&penaltyName=${bean.penaltyName}&path=${bean.path}&state=${bean.state}&register=${bean.register}"
												aria-label="Next"> <span aria-hidden="true">&raquo;</span>
													<span class="sr-only">Next</span>
											</a></li>
										</c:otherwise>
		...
		$(function() {
			...
			$('#selectNum')
					.on(
							...
							function() {
								...
								location.href = '${root}/account_manage/shop_penalty_list?dateSelete=${bean.dateSelete}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyword=${bean.keyword}&franchiseId=${bean.franchiseId}&id=${bean.id}&penaltyName=${bean.penaltyName}&path=${bean.path}&state=${bean.state}&register=${bean.register}&selectNum='
										+ selectNum;
							});
			...
			$('.question-table tbody tr td').not('.tableTd').on(
					...
					function() {
						...
						let shopPenaltyId = $(this).parent().data('shopPenaltyId');
						window.open(
								"${root}/account_manage/shop_penalty_detail?shopPenaltyId="
										+ shopPenaltyId, "_blank",
