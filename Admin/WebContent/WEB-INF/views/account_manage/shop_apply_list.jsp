            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <form method="get" action="${root}/account_manage/shop_apply_list">
							...
							<div class="card-body">
								...
								<div class="row">
									<div class="col-sm-1">심사상태</div>
									<div class="col-sm-4">
										<select class="form-control" name="activityStatus">
											<option value="1" <c:if test="${bean.activityStatus eq 1}">selected</c:if>>전체</option>
											<option value="2" <c:if test="${bean.activityStatus eq 2}">selected</c:if>>심사중</option>
											<option value="3" <c:if test="${bean.activityStatus eq 3}">selected</c:if>>반려</option>
											<option value="4" <c:if test="${bean.activityStatus eq 4}">selected</c:if>>보류</option>
										</select>
									</div>
									<div class="col-sm-1">
										신청 정보
									...
									<div class="col-sm-2">
										<select class="form-control" name="select">
											...
											<option value="1" <c:if test="${bean.select eq 1}">selected</c:if>>신청 가게명</option>
											<option value="2" <c:if test="${bean.select eq 2}">selected</c:if>>닉네임</option>
											<option value="3" <c:if test="${bean.select eq 3}">selected</c:if>>등록 키워드</option>
										</select>
									</div>
									<div class="col-sm-3">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table question-table" id="franchiseListTable" style="margin-top:1rem;">
                                ...
                                <tbody>
                              	    <fmt:parseNumber value="${toDay.time / (1000*60*60*24)}" integerOnly="true" var="today_"/>
				          		...
				          		<c:forEach var="idx" begin="${pageBean.min}" end="${pageBean.max}">
				          			<c:choose>
				          				<c:when test="${idx == pageBean.page}">
				          					<li class="page-item active"><a class="page-link" href="${root}/account_manage/shop_apply_list?page=${idx}&selectNum=${selectNum}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyWord=${bean.keyWord}&franchiseId=${bean.franchiseId}&id=${bean.id}&sectorId=${bean.sectorId}&penalty=${bean.penalty}&activityStatus=${bean.activityStatus}">${idx}</a></li>
				          				</c:when>
				          				<c:otherwise>
				          					<li class="page-item"><a class="page-link" href="${root}/account_manage/shop_apply_list?page=${idx}&selectNum=${selectNum}&startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyWord=${bean.keyWord}&franchiseId=${bean.franchiseId}&id=${bean.id}&sectorId=${bean.sectorId}&penalty=${bean.penalty}&activityStatus=${bean.activityStatus}">${idx}</a></li>
				          				</c:otherwise>
				          			</c:choose>	
				          		</c:forEach>
				          		<c:choose>
	...
	<script>
		$(function(){
   			...
   			$('#selectNum').on('change',function(){
   				...
   				location.href='${root}/account_manage/shop_apply_list?startDate=${bean.startDate}&endDate=${bean.endDate}&select=${bean.select}&keyWord=${bean.keyWord}&franchiseId=${bean.franchiseId}&id=${bean.id}&sectorId=${bean.sectorId}&penalty=${bean.penalty}&activityStatus=${bean.activityStatus}&selectNum='+selectNum;
   			});
   			
   			//가게 상세 페이지 이동
   			$('.question-table tbody tr td').not('.tableTd').on('click',function(){
   				
   				let shopIdx = $(this).parent().data('shopIdx');
   				window.open("${root}/account_manage/shop_apply_detail?shopIdx="+shopIdx,"_blank",
