            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <form method="get" action="${root}/manager/manager_activity">
								...
								<div class="row">
									...
									<div class="col-sm-3">
										<select class="form-control" style="width:84%;" name="select">
											...
											<option value="2"<c:if test="${map.select eq 2}">selected</c:if>>수행 내용</option>
											<option value="3"<c:if test="${map.select eq 3}">selected</c:if>>이름</option>
											<option value="4"<c:if test="${map.select eq 4}">selected</c:if>>아이디</option>
										</select>
									</div>
									<div class="col-sm-8">
                            ...
                            <table class="table" id="franchiseListTable" style="margin-top:1rem;">
                              ...
                              <div class="row justify-content-md-center">
				          		...
				          		<c:choose>
				          			...
				          			<c:otherwise>
				          				<li class="page-item">
				          				<a class="page-link" href="${root}/manager/manager_activity?page=${pageBean.prevPage}&keyWord=${map.keyWord}&startDate=${map.startDate}&endDate=${map.endDate}&select=${map.select}&selectNum=${map.selectNum}" aria-label="Previous">
										        <span aria-hidden="true">&laquo;</span>
										        <span class="sr-only">Previous</span>
									      	</a></li>
				          			</c:otherwise>
				          		...
				          		<c:forEach var="idx" begin="${pageBean.min}" end="${pageBean.max}">
				          			<c:choose>
				          				<c:when test="${idx == pageBean.page}">
				          					<li class="page-item active"><a class="page-link" href="${root}/manager/manager_activity?page=${idx}&keyWord=${map.keyWord}&startDate=${map.startDate}&endDate=${map.endDate}&select=${map.select}&selectNum=${map.selectNum}">${idx}</a></li>
				          				</c:when>
				          				<c:otherwise>
				          					<li class="page-item"><a class="page-link" href="${root}/manager/manager_activity?page=${idx}&keyWord=${map.keyWord}&startDate=${map.startDate}&endDate=${map.endDate}&select=${map.select}&selectNum=${map.selectNum}">${idx}</a></li>
				          				</c:otherwise>
				          			</c:choose>	
				          		</c:forEach>
				          		<c:choose>
				          			...
				          			<c:otherwise>
				          				<li class="page-item">
										      <a class="page-link" href="${root}/manager/manager_activity?page=${pageBean.nextPage}&keyWord=${map.keyWord}&startDate=${map.startDate}&endDate=${map.endDate}&select=${map.select}&selectNum=${map.selectNum}" aria-label="Next">
										        <span aria-hidden="true">&raquo;</span>
										        <span class="sr-only">Next</span>
										      </a>
										    	</li>
	...
	<script>
		$(function(){
   			...
   			$('#selectNum').on('change',function(){
   				...
   				location.href='${root}/manager/manager_activity?keyWord=${map.keyWord}&startDate=${map.startDate}&endDate=${map.endDate}'+
