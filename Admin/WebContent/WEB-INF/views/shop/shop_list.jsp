...
<body id="page-top">
	                        ...
	                        <table class="table table-bordered"> 
	                        	...
	                        	<tbody>
	                        		<tr>
	                        			...
	                        			<td colspan="3">
	                        				<select class="form-control" style="width:20%;display:inline-block;" name="dateSelect">
	                        					<option value = "1" <c:if test="${shopSearchBean.dateSelect eq '1'}">selected</c:if>>승인일</option>
	                        					<option value = "2" <c:if test="${shopSearchBean.dateSelect eq '2'}">selected</c:if>>신청일</option>
	                        					<option value = "3" <c:if test="${shopSearchBean.dateSelect eq '3'}">selected</c:if>>탈퇴일</option>
	                        				</select>
	                        				<input type="date" style="margin-left:20px;width:30%;display:inline-block;" class="form-control" name="dateData" value="${shopSearchBean.dateData}">
	                        			</td>
	                        		</tr>
	                        		<tr>
	                        			<th scope="row">가맹사</th>
    ...
    <script>
		$(function(){
			...
			// 이미지에 마우스 오버 시 이미지 확대 
			$('.imgScale').mouseover(function(){
				$(this).find('img').css('transform','scale(2)');
				$(this).find('img').css('transition','all 0.3s ease-in-out');
			});
			
			$('.imgScale').mouseout(function() {
				$(this).find('img').css('transform','');
				$(this).find('img').css('transition','');
			})
			
			// 컬럼 클릭 시 그 컬럼 기준으로 정렬 
